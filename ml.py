...
TEXT_LIMIT = 5000
CHUNK_SIZE = 3000
PROMPT_LIMIT = 3400  # 3400


def text_processing(raw_text: str) -> str:
    # with open(filename, encoding="utf-8") as text_file:
    #     raw_text = text_file.read()
    if len(raw_text) > PROMPT_LIMIT:
        chunks = [raw_text[index: index + CHUNK_SIZE]
                  for index in range(0, len(raw_text), CHUNK_SIZE)]
        text = "\n".join(
...
def create_summary_prompt(text: str, filename: str) -> list[str] | None:
    ...
    prompt += (
        ...
        f"чем ты можешь написать за одно сообщение, я буду ждать продолжение в следующем твоём сообщении. Когда закончишь, "
        f'напиши "КОНЕЦ"'
    )
    prompt += "\n\n" + text
...
def make_seo_optimization(text: str, filename: str = "prompts/PROMPT{}.txt") -> list[str] | None:
    """
    text - предобработанный текст (после Edge)
    """
    prompts = []
    with open(filename.format(1), "r") as file:
        prompt = file.read()
...
if __name__ == "__main__":
    ...
    print(test_text, end="\n\n-----\n\n")
    test_prompts = create_summary_prompt(test_text, "Prompt.txt")
